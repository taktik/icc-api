...
class IccContactXApi extends iccApi_1.iccContactApi {
  constructor(
    host,
    headers,
    crypto,
  ...
  filterByWithUser(user, startKey, startDocumentId, limit, body) {
    return super
      .filterBy(startKey, startDocumentId, limit, body)
      .then(ctcs =>
                  ...
                  Promise.all(
                    ...
                    .then(eSvcs => {
                      ...
                      ctc.services.forEach((svc, idx) => {
                        svc.encryptedSelf = btoa(utils_1.utils.ua2text(eSvcs[idx]))
                        delete svc.content
                      })
                    })
                    .then(() =>
                    ...
                    .then(es => {
                      ctc.encryptedSelf = btoa(utils_1.utils.ua2text(es))
                      delete ctc.descr
                      return ctc
                    })
                )
                          ...
                          svc.encryptedContent
                            ? this.crypto.AES.decrypt(
                                ...
                                c => {
                                  let jsonContent
                                  try {
                                    jsonContent = utils_1.utils.ua2utf8(c).replace(/\0+$/g, "")
                            ...
                            : svc.encryptedSelf
                              ? this.crypto.AES.decrypt(
                                  ...
                                  s => {
                                    let jsonContent
                                    try {
                                      jsonContent = utils_1.utils.ua2utf8(s).replace(/\0+$/g, "")
          ...
          .then(
            ({ extractedKeys: sfks }) =>
              svc.encryptedContent || svc.encryptedSelf
                ? this.crypto.AES.importKey("raw", utils_1.utils.hex2ua(sfks[0].replace(/-/g, "")))
                    .then(
                      key =>
                        new Promise(resolve => {
                          svc.encryptedContent
