...
class IccCryptoXApi {
  constructor(
    ...
    this._utils = new utils_1.UtilsClass(crypto)
  ...
  getCachedHcpOrPatientType(hcpartyId) {
      ...
      .then(
        ([
          ...
          const cryptedForeignKeysCryptedDecrypted = _.merge(
            ((modifiedObject.cryptedForeignKeys || {})[delegateId] || []).map(d => ({ d })),
            (previousDecryptedCryptedForeignKeys || [])
              .map(dd => (dd ? this._utils.ua2text(dd) : null))
              .map(k => ({ k }))
          )
            ...
            ((modifiedObject.encryptionKeys || {})[delegateId] || []).map(
              d =>
                (d.key &&
                  d.owner === ownerId &&
                  this._AES
                    .decrypt(decryptedHcPartyKey.key, this._utils.hex2ua(d.key))
                    .catch(() => {
                      console.log(
        ...
        const genProm = new Promise((resolve, reject) => {
          delegate.publicKey
            ? this._AES
                ...
                .then(AESKey => {
                  const ownerPubKey = utils_1.utils.spkiToJwk(utils_1.utils.hex2ua(owner.publicKey))
