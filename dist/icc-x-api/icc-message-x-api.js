...
const fhc_api_2 = require("fhc-api")
class EfactSendResponseWithError extends fhc_api_1.EfactSendResponse {
  constructor(json) {
    super(json)
  }
}
class IccMessageXApi extends iccApi_1.iccMessageApi {
  constructor(
    host,
    headers,
    crypto,
  ...
  saveDmgsListRequest(user, req, requestDate) {
    return this.newInstance(user, {
      ...
      transportGuid:
        "GMD:OUT:" +
        ((req.commonOutput && req.commonOutput.inputReference) || req.tack.appliesTo || "").replace(
          "urn:nip:reference:input:",
          ""
        ),
        ...
        return this.documentXApi
          ...
          .then(doc =>
            this.documentXApi.setAttachment(
              ...
              utils_1.utils.ua2ArrayBuffer(utils_1.utils.text2ua(JSON.stringify(req)))
            )
          )
          .then(() => msg)
      })
        ...
        _.chunk(Object.keys(patsDmgs), 100).map(ssins =>
          this.patientApi
            .filterByWithUser(
              ...
              undefined,
              false,
              new models_1.FilterChain({
                filter: new models_1.Filter({
                ...
                (pats.rows || []).map(p => {
                  ...
                  if (!phcp) {
                    p.patientHealthCareParties.push(
                      (phcp = new models_1.PatientHealthCarePartyDto({
                        healthcarePartyId: user.healthcarePartyId,
                        referralPeriods: []
                      }))
                    )
                  ...
                  const rp =
                    ...
                    (phcp.referralPeriods[
                      phcp.referralPeriods.length
                    ] = new models_1.ReferralPeriod({}))
                  ...
                  if (latestAction) {
                    if (latestAction.closure) {
                      ...
                      if (actionDate > (rp.startDate || 0)) {
                        rp.endDate = actionDate
                        phcp.referralPeriods.push(
                          new models_1.ReferralPeriod({ startDate: actionDate })
                        )
                      }
                    }
                  }
  ...
  makeHcp(hcParty) {
    ...
    return `${hcParty.firstname || ""} ${hcParty.familyname || ""} ${hcParty.name ||
      ""} [${(hcParty.ids &&
      (hcParty.ids.find(id => id.s === fhc_api_2.IDHCPARTY.SEnum.IDHCPARTY) || {}).value) ||
      "-"}]`
  }
        ...
        return docXApi
          ...
          .then(doc =>
            docXApi.setAttachment(
              doc.id,
              undefined /*TODO provide keys for encryption*/,
              utils_1.utils.ua2ArrayBuffer(utils_1.utils.text2ua(JSON.stringify(dmgMessage)))
            )
          )
          .then(() => msg)
      })
  ...
  processTack(user, hcp, efactMessage) {
    ...
    return this.findMessagesByTransportGuid(
      ...
      return this.receiptXApi
        ...
        .then(rcpt =>
          this.receiptXApi.setAttachment(
            rcpt.id,
            "tack",
            undefined,
            utils_1.utils.ua2ArrayBuffer(utils_1.utils.text2ua(JSON.stringify(efactMessage)))
          )
        )
        .then(() => {
  ...
  processEfactMessage(user, hcp, efactMessage, invoicePrefix, invoicePrefixer) {
    ...
    return this.findMessagesByTransportGuid(
      ...
      const parentMessage = fileReference
        ? _.find(msgsForHcp, m => efact_util_1.uuidBase36(m.id) === fileReference.trim())
        : msgsForHcp[0]
      if (!parentMessage) {
      ...
      const invoicingErrors = parsedRecords.records
        ? _.compact(
            _.flatMap(parsedRecords.records, r => {
              ...
              if (r.et20 && r.et20.errorDetail) {
                errors.push({
                  itemId: efact_util_1.decodeBase36Uuid(refEt20),
              ...
              _.each(r.items, i => {
                ...
                if (i.et50 && i.et50.errorDetail) {
                  errors.push({
                    itemId: ref && efact_util_1.decodeBase36Uuid(ref),
      ...
      return this.newInstance(user, {
        ...
        fromAddress: "EFACT",
        sent: formatting_util_1.timeEncode(new Date()),
        ...
        .then(msg =>
          Promise.all([
            ...
            .then(([doc, jsonDoc, jsonParsedDoc]) =>
              Promise.all([
                this.documentXApi.setAttachment(
                  doc.id,
                  undefined /*TODO provide keys for encryption*/,
                  utils_1.utils.ua2ArrayBuffer(utils_1.utils.text2ua(efactMessage.detail))
                ),
                this.documentXApi.setAttachment(
                  jsonDoc.id,
                  undefined /*TODO provide keys for encryption*/,
                ...
                this.documentXApi.setAttachment(
                  jsonParsedDoc.id,
                  undefined /*TODO provide keys for encryption*/,
                  utils_1.utils.ua2ArrayBuffer(utils_1.utils.text2ua(JSON.stringify(parsedRecords)))
                )
              ])
            )
            .then(
              () =>
                ["920999", "920099", "920900"].includes(messageType)
                  ? this.invoiceXApi.getInvoices(
                      new models_1.ListOfIdsDto({ ids: parentMessage.invoiceIds })
                    )
                  : Promise.resolve([])
            )
            .then(invoices => {
                      ...
                      _.flatMap(parsedRecords.records, r =>
                        r.items.map(
                          i =>
                            ...
                            efact_util_1.decodeBase36Uuid(i.et50.itemReference.trim()) === ic.id &&
                            i.et50
                        )
                      )
                    )[0]
            ...
            .then(invoices => {
              ...
              return this.modifyMessage(parentMessage).then(message => ({ message, invoices }))
            })
        )
    })
  }
  sendBatch(
    ...
    fhcServer = undefined,
    prefixer,
    isConnectedAsPmg = false,
    medicalLocationId
  ) {
    const uuid = this.crypto.randomUuid()
    const smallBase36 = efact_util_1.uuidBase36Half(uuid)
    const fullBase36 = efact_util_1.uuidBase36(uuid)
    const sentDate = +new Date()
    const errors = []
    const year = moment().year()
    return efact_util_1.getFederaton(invoices, this.insuranceApi).then(fed => {
      return (prefixer
        ? prefixer(fed, hcp.id)
      ...
      .then(([jsonDoc, doc]) =>
        Promise.all([
          this.documentXApi.setAttachment(
            jsonDoc.id,
            undefined /*TODO provide keys for encryption*/,
            utils_1.utils.ua2ArrayBuffer(utils_1.utils.text2ua(JSON.stringify(res.records)))
          ),
          this.documentXApi.setAttachment(
            doc.id,
            undefined /*TODO provide keys for encryption*/,
            utils_1.utils.ua2ArrayBuffer(utils_1.utils.text2ua(res.detail))
          )
        ])
      )
      .then(() =>
        this.receiptXApi.logReceipt(
          ...
          "tack",
          utils_1.utils.ua2ArrayBuffer(utils_1.utils.text2ua(JSON.stringify(res.tack)))
        )
      )
  }
}
