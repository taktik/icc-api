...
class iccTimeTableApi {
  ...
  createTimeTable(body) {
    ...
    return XHR_1.XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => new models.TimeTableDto(doc.body))
  ...
  deleteTimeTable(timeTableIds) {
    ...
    const _url =
      this.host +
      "/timeTable/{timeTableIds}".replace("{timeTableIds}", timeTableIds + "") +
      "?ts=" +
      new Date().getTime()
    let headers = this.headers
    headers = headers
  ...
  getTimeTable(timeTableId) {
    ...
    const _url =
      this.host +
      "/timeTable/{timeTableId}".replace("{timeTableId}", timeTableId + "") +
      "?ts=" +
      new Date().getTime()
    let headers = this.headers
    headers = headers
  ...
  getTimeTablesByAgendaId(agendaId) {
    ...
    headers = headers
      ...
      .concat(new XHR_1.XHR.Header("Content-Type", "application/json"))
    return XHR_1.XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => doc.body.map(it => new models.TimeTableDto(it)))
      .catch(err => this.handleError(err))
  }
