...
class iccBeResultImportApi {
  ...
  doImport(documentId, hcpId, language, protocolIds, formIds, planOfActionId, enckeys, body) {
    let _body = null
    _body = body
    const _url =
      this.host +
  ...
  getInfos(id, full, language, enckeys) {
    let _body = null
    const _url =
      this.host +
    ...
    headers = headers
      ...
      .concat(new XHR_1.XHR.Header("Content-Type", "application/json"))
    return XHR_1.XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc => doc.body.map(it => new models.ResultInfoDto(it)))
      .catch(err => this.handleError(err))
  }
}
