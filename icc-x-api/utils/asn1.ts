...
export class Int10 {
  static max = 10000000000000 // biggest integer that can still fit 2^53 when multiplied by 256
  ...
  toString(base = 10) {
    ...
    for (let i = b.length - 2; i >= 0; --i) s += (Int10.max + b[i]).toString().substring(1)
    return s
  }
  ...
  valueOf() {
    let b = this.buf,
      v = 0
    for (let i = b.length - 1; i >= 0; --i) v = v * Int10.max + b[i]
    return v
  }
...
export class Stream {
  ...
  hexDump(start: number, end: number, raw = false) {
    let s = ''
...
export class ASN1 {
  ...
  static decode(stream: Stream) {
    let streamStart = new Stream(stream.enc, stream.pos),
