...
export class iccDocumentApi {
  ...
  findByTypeHCPartyMessageSecretFKeys(documentTypeCode?: string, hcPartyId?: string, secretFKeys?: string): Promise<Array<models.DocumentDto> | any> {
    let _body = null
    
    const _url = this.host + "/document/byTypeHcPartySecretForeignKeys" + "?ts=" + new Date().getTime()  + (documentTypeCode ? "&documentTypeCode=" + documentTypeCode : "") + (hcPartyId ? "&hcPartyId=" + hcPartyId : "") + (secretFKeys ? "&secretFKeys=" + secretFKeys : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc => (doc.body as Array<JSON>).map(it => new models.DocumentDto(it)))
  ...
  getAttachment(documentId: string, attachmentId: string, enckeys?: string, fileName?: string): Promise<ArrayBuffer | any> {
    let _body = null
    
    const _url = this.host + "/document/{documentId}/attachment/{attachmentId}".replace("{documentId}", documentId+"").replace("{attachmentId}", attachmentId+"") + "?ts=" + new Date().getTime()  + (enckeys ? "&enckeys=" + enckeys : "") + (fileName ? "&fileName=" + fileName : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc =>  doc.body)
  ...
  setAttachment(documentId: string, enckeys?: string, body?: Array<string>): Promise<models.DocumentDto | any> {
    let _body = null
    _body = body
    
    const _url = this.host + "/document/{documentId}/attachment".replace("{documentId}", documentId+"") + "?ts=" + new Date().getTime()  + (enckeys ? "&enckeys=" + enckeys : "")
    let headers = this.headers
  ...
  setAttachmentMulti(documentId: string, enckeys?: string, attachment?: Array<string>): Promise<models.DocumentDto | any> {
    let _body = null
    ;attachment && (_body = (_body || new FormData())).append('attachment', new Blob(attachment, {type: "application/octet-stream"}))
    const _url = this.host + "/document/{documentId}/attachment/multipart".replace("{documentId}", documentId+"") + "?ts=" + new Date().getTime()  + (enckeys ? "&enckeys=" + enckeys : "")
    let headers = this.headers
