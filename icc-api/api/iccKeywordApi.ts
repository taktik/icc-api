...
export class iccKeywordApi {
  ...
  deleteKeywords(keywordIds: string): Promise<Array<string> | any> {
    ...
    const _url = this.host + "/keyword/{keywordIds}".replace("{keywordIds}", keywordIds+"") + "?ts=" + new Date().getTime() 
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
  ...
  getKeyword(keywordId: string): Promise<models.KeywordDto | any> {
    ...
    const _url = this.host + "/keyword/{keywordId}".replace("{keywordId}", keywordId+"") + "?ts=" + new Date().getTime() 
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
  ...
  getKeywordByUser(userId: string): Promise<models.KeywordDto | any> {
    ...
    const _url = this.host + "/keyword/byUser/{userId}".replace("{userId}", userId+"") + "?ts=" + new Date().getTime() 
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("GET", _url, headers, _body)
      .then(doc =>  new models.KeywordDto(doc.body as JSON))
