...
export class iccAccesslogApi {
  ...
  findByHCPartyPatientSecretFKeys(hcPartyId?: string, secretFKeys?: string): Promise<Array<models.AccessLogDto> | any> {
    let _body = null
    
    const _url = this.host + "/accesslog/byHcPartySecretForeignKeys" + "?ts=" + new Date().getTime()  + (hcPartyId ? "&hcPartyId=" + hcPartyId : "") + (secretFKeys ? "&secretFKeys=" + secretFKeys : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
  ...
  listAccessLogs(fromEpoch?: number, toEpoch?: number, startKey?: string, startDocumentId?: string, limit?: string, descending?: boolean): Promise<models.AccessLogPaginatedList | any> {
    let _body = null
    
    const _url = this.host + "/accesslog" + "?ts=" + new Date().getTime()  + (fromEpoch ? "&fromEpoch=" + fromEpoch : "") + (toEpoch ? "&toEpoch=" + toEpoch : "") + (startKey ? "&startKey=" + startKey : "") + (startDocumentId ? "&startDocumentId=" + startDocumentId : "") + (limit ? "&limit=" + limit : "") + (descending ? "&descending=" + descending : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc =>  new models.AccessLogPaginatedList(doc.body as JSON))
