...
export class iccMessageApi {
  ...
  findMessagesByTransportGuid(transportGuid?: string, received?: boolean, startKey?: string, startDocumentId?: string, limit?: number): Promise<models.MessagePaginatedList | any> {
    let _body = null
    
    const _url = this.host + "/message/byTransportGuid" + "?ts=" + new Date().getTime()  + (transportGuid ? "&transportGuid=" + transportGuid : "") + (received ? "&received=" + received : "") + (startKey ? "&startKey=" + startKey : "") + (startDocumentId ? "&startDocumentId=" + startDocumentId : "") + (limit ? "&limit=" + limit : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("GET", _url, headers, _body)
      .then(doc =>  new models.MessagePaginatedList(doc.body as JSON))
