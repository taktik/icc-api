...
export class iccDocumentApi {
  ...
  getAttachment(documentId: string, attachmentId: string, enckeys?: string): Promise<any | Boolean> {
    let _body = null
    
    const _url = this.host + "/document/{documentId}/attachment/{attachmentId}".replace("{documentId}", documentId+"").replace("{attachmentId}", attachmentId+"") + "?ts=" + new Date().getTime()  + (enckeys ? "&enckeys=" + enckeys : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("GET", _url, headers, _body)
