...
export class iccContactApi {
  ...
  findByHCPartyPatientForeignKeys(hcPartyId?: string, body?: models.ListOfIdsDto): Promise<Array<models.ContactDto> | any> {
    let _body = null
    _body = body
    
    const _url = this.host + "/contact/byHcPartyPatientForeignKeys" + "?ts=" + new Date().getTime()  + (hcPartyId ? "&hcPartyId=" + hcPartyId : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => (doc.body as Array<JSON>).map(it => new models.ContactDto(it)))
  ...
  findDelegationsStubsByHCPartyPatientSecretFKeys(hcPartyId?: string, secretFKeys?: string): Promise<Array<models.ContactDto> | any> {
    let _body = null
    
    const _url = this.host + "/contact/byHcPartySecretForeignKeys/delegations" + "?ts=" + new Date().getTime()  + (hcPartyId ? "&hcPartyId=" + hcPartyId : "") + (secretFKeys ? "&secretFKeys=" + secretFKeys : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc => (doc.body as Array<JSON>).map(it => new models.ContactDto(it)))
  ...
  getServiceCodesOccurences(codeType: string, minOccurences: number): Promise<Array<models.LabelledOccurenceDto> | any> {
    let _body = null
    
    const _url = this.host + "/contact/service/codes/{codeType}/{minOccurences}".replace("{codeType}", codeType+"").replace("{minOccurences}", minOccurences+"") + "?ts=" + new Date().getTime() 
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
  ...
  listContactsByOpeningDate(startKey: number, endKey: number, hcpartyid: string, startDocumentId?: string, limit?: number): Promise<models.ContactPaginatedList | any> {
    let _body = null
    
    const _url = this.host + "/contact/byOpeningDate" + "?ts=" + new Date().getTime()  + (startKey ? "&startKey=" + startKey : "") + (endKey ? "&endKey=" + endKey : "") + (hcpartyid ? "&hcpartyid=" + hcpartyid : "") + (startDocumentId ? "&startDocumentId=" + startDocumentId : "") + (limit ? "&limit=" + limit : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("GET", _url, headers, _body, this.fetchImpl)
      .then(doc =>  new models.ContactPaginatedList(doc.body as JSON))
