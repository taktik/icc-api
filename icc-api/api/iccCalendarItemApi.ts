...
export class iccCalendarItemApi {
  ...
  findByHCPartyPatientSecretFKeys(hcPartyId?: string, secretFKeys?: string): Promise<Array<models.CalendarItemDto> | any> {
    let _body = null
    
    const _url = this.host + "/calendarItem/byHcPartySecretForeignKeys" + "?ts=" + new Date().getTime()  + (hcPartyId ? "&hcPartyId=" + hcPartyId : "") + (secretFKeys ? "&secretFKeys=" + secretFKeys : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
  ...
  getCalendarItemsByPeriodAndHcPartyId(startDate?: number, endDate?: number, hcPartyId?: string): Promise<Array<models.CalendarItemDto> | any> {
    let _body = null
    
    const _url = this.host + "/calendarItem/byPeriodAndHcPartyId" + "?ts=" + new Date().getTime()  + (startDate ? "&startDate=" + startDate : "") + (endDate ? "&endDate=" + endDate : "") + (hcPartyId ? "&hcPartyId=" + hcPartyId : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => (doc.body as Array<JSON>).map(it => new models.CalendarItemDto(it)))
  ...
  getCalendarItemsWithIds(body?: models.ListOfIdsDto): Promise<Array<models.CalendarItemDto> | any> {
    let _body = null
    _body = body
  ...
  getCalendarsByPeriodAndAgendaId(startDate?: number, endDate?: number, agendaId?: string): Promise<Array<models.CalendarItemDto> | any> {
    let _body = null
    
    const _url = this.host + "/calendarItem/byPeriodAndAgendaId" + "?ts=" + new Date().getTime()  + (startDate ? "&startDate=" + startDate : "") + (endDate ? "&endDate=" + endDate : "") + (agendaId ? "&agendaId=" + agendaId : "")
    let headers = this.headers
    headers = headers.filter(h => h.header !== "Content-Type").concat(new XHR.Header("Content-Type", "application/json"))
    return XHR.sendCommand("POST", _url, headers, _body, this.fetchImpl)
      .then(doc => (doc.body as Array<JSON>).map(it => new models.CalendarItemDto(it)))
